...
use std::str::FromStr;
use taskchampion::{utc_timestamp, Operations, Replica, ServerConfig, Status, StorageConfig, Tag};
use uuid::Uuid;
        ...
        if !is_known {
            ...
            if key == "scheduled" || key == "until" {
                if let Some(dt) = parse_datetime(value) {
                    task.set_legacy_uda(key.clone(), dt.to_rfc3339(), &mut ops)?;
                }
            } else if key == "urgency" {
                // Store urgency as-is (it's typically calculated, but can be stored)
                task.set_legacy_uda(key.clone(), value.clone(), &mut ops)?;
            } else {
...
fn task_to_map(task: &taskchampion::Task) -> HashMap<String, String> {
    ...
    for (key, value) in task.get_legacy_udas() {
...
pub fn add_task(
    ...
    let mut replica = Replica::new(storage);
    let uuid = create_task_from_map(&mut replica, task_data)?;
...
pub fn update_task(
    ...
    update_task_in_replica(&mut replica, uuid, task_data)?;
    ...
    let tasks: Vec<taskchampion::Task> =
        if let FilterExpression::EqualsFilter { property, value } = &filter {
            if property.name == "status" && value.as_str() == Some("pending") {
                ...
                replica.pending_tasks()?.into_iter().collect()
            } else {
                // Fall back to all_tasks for other filters
                replica.all_tasks()?.into_values().collect()
            }
        } else {
...
pub fn sync_with_server(
    ...
    let num_local_operations = replica.num_local_operations().unwrap_or(0) as u64;
...
pub fn get_snapshot(
    ...
    if let Some((version_id, _snapshot_data)) = server.get_snapshot()? {
    ...
    for task_data in tasks_data {
        ...
        if let Some(uuid_str) = task_data.get("uuid") {
            if let Ok(uuid) = Uuid::parse_str(uuid_str) {
                if replica.get_task(uuid)?.is_some() {
                    continue;
                }
            }
        }

        if create_task_from_map(&mut replica, task_data).is_ok() {
            imported_count += 1;
        }
    }
...
mod tests {
    ...
    fn create_test_task_with_project(
        replica: &mut Replica,
        description: &str,
        project: &str,
    ) -> Uuid {
        ...
        task.set_description(description.to_string(), &mut ops)
            .unwrap();
        task.set_legacy_uda("project".to_string(), project.to_string(), &mut ops)
            .unwrap();
        replica.commit_operations(ops).unwrap();
        uuid
    }
    ...
    fn test_get_string_property_description() {
        ...
        let uuid = create_test_task(&mut replica, "Test task", Status::Pending, "");
    ...
    fn test_get_string_property_status() {
        ...
        let uuid = create_test_task(&mut replica, "Test task", Status::Completed, "");
        let task = replica.get_task(uuid).unwrap().unwrap();
    ...
    fn test_get_string_property_priority() {
        ...
        let uuid = create_test_task(&mut replica, "Test task", Status::Pending, "H");
        let task = replica.get_task(uuid).unwrap().unwrap();
    ...
    fn test_get_string_property_project() {
        ...
        let uuid = create_test_task_with_project(&mut replica, "Test task", "MyProject");
        let task = replica.get_task(uuid).unwrap().unwrap();
    ...
    fn test_get_datetime_property_due() {
        ...
        let uuid = create_test_task_with_due(&mut replica, "Test task", due_date);
        let task = replica.get_task(uuid).unwrap().unwrap();
    ...
    #[test]
    fn test_get_datetime_property_entry() {
        let temp_dir = TempDir::new().unwrap();
        let storage = StorageConfig::OnDisk {
            taskdb_dir: temp_dir.path().to_path_buf(),
            create_if_missing: true,
    ...
    fn test_has_virtual_tag_tagged() {
        ...
        let uuid = create_test_task_with_tags(&mut replica, "Test task", vec!["home", "important"]);
        let task = replica.get_task(uuid).unwrap().unwrap();
    ...
    #[test]
    fn test_has_virtual_tag_untagged() {
        let temp_dir = TempDir::new().unwrap();
        let storage = StorageConfig::OnDisk {
            taskdb_dir: temp_dir.path().to_path_buf(),
            create_if_missing: true,
    ...
    fn test_evaluate_contains_filter() {
        ...
        let uuid = create_test_task(&mut replica, "Buy milk from store", Status::Pending, "");
        let task = replica.get_task(uuid).unwrap().unwrap();
    ...
    fn test_evaluate_starts_with_filter() {
        ...
        let uuid = create_test_task(&mut replica, "Buy milk", Status::Pending, "");
        let task = replica.get_task(uuid).unwrap().unwrap();
    ...
    fn test_evaluate_any_filter() {
        ...
        let uuid2 = create_test_task(&mut replica, "Test task 2", Status::Pending, "");
        let task2 = replica.get_task(uuid2).unwrap().unwrap();
    ...
    fn test_evaluate_less_than_filter() {
        ...
        task.set_description("Test task".to_string(), &mut ops)
            .unwrap();
        task.set_legacy_uda("urgency".to_string(), "5.0".to_string(), &mut ops)
            .unwrap();
        replica.commit_operations(ops).unwrap();
    ...
    fn test_get_tasks_with_filter_json_status_pending() {
        ...
        // Create test tasks
        create_test_task(&mut replica, "Pending task 1", Status::Pending, "");
        create_test_task(&mut replica, "Pending task 2", Status::Pending, "");
        create_test_task(&mut replica, "Completed task", Status::Completed, "");
        ...
        let result = get_tasks_with_filter_json(
    ...
    fn test_get_tasks_with_filter_json_complex_filter() {
        ...
        // Create test tasks
        create_test_task_with_project(&mut replica, "Task 1", "ProjectA");
        create_test_task_with_project(&mut replica, "Task 2", "ProjectB");
        create_test_task_with_project(&mut replica, "Task 3", "ProjectA");
        create_test_task(&mut replica, "Task 4", Status::Completed, "");
